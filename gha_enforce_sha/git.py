...
def init_repo_from_action(repo_path: str, action: str) -> None:
    must_run_git_cmd("git", "init", "--bare", repo_path)
    must_run_git_cmd("git", "-C", repo_path, "remote", "add", _REMOTE_NAME)


def resolve_tag(repo_path: str, partial_tag: str) -> tuple[str, str]:
    must_run_git_cmd(
        "-C",
        "repo_path",
        "fetch",
    ...
    if retcode == 0:
        ...
        return partial_tag, stdout

    # sort tags by version
    stdout = must_run_git_cmd(
        "-C", repo_path, "tag", "--list", "--short=-version:refname", f"{partial_tag}*"
    )
...
def _cmd_output(*cmd: str) -> tuple[int, str, str]:
    ...
    return proc.returncode, str(proc.stdout), str(proc.stderr)
